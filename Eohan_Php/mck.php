...
SELECT * FROM 
        ...
        WHERE guangyun.homophone = gyhomophone.id ) emc 
        LEFT JOIN gsr ON emc.codepoint = gsr.codepoint 
        WHERE emc.mck = ? ) AS readings , glyph 
...
$stmt->execute(array($reading,$reading,$reading,$reading,$reading,$reading,$reading,$reading)); 
$rows = $stmt->fetchAll();
$db=null;
$mckReading = $reading;
...
$codepoint = $row[0];
$glyph = $row[1];
if(!key_exists($codepoint,$mckDict)){
    $mckDict[$codepoint] = ["glyph" => $glyph,
                                 "codepoint" => $codepoint,
                                 "em" => [],
                                 "lmc" => [],
                                 "emc" => [],
...
$reading = $row[2];
$period = $row[3];
array_push($mckDict[$codepoint][$period],$reading);
} // end for

include 'top.php';

$text = <<<TEXT
<h2 class="text" id="glyphheader" align=center>Middle Chinese: Karlgren</h2>
...
foreach ($mckDict as $codepoint => $glypharray){
