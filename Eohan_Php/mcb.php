...
SELECT * FROM 
        ...
        WHERE guangyun.homophone = gyhomophone.id ) emc 
        LEFT JOIN gsr ON emc.codepoint = gsr.codepoint 
        WHERE emc.mcb = ? ) AS readings , glyph 
...
$codepoint = $row[0];
$glyph = $row[1];
if(!key_exists($codepoint,$mcbDict)){
    $mcbDict[$codepoint] = ["glyph" => $glyph,
                                 "codepoint" => $codepoint,
                                 "em" => [],
                                 "lmc" => [],
                                 "emc" => [],
...
$reading = $row[2];
$period = $row[3];
array_push($mcbDict[$codepoint][$period],$reading);
} // end for

include 'top.php';

$text = <<<TEXT
<h2 class="text" id="glyphheader" align=center>Middle Chinese: Baxter</h2>
...
foreach ($mcbDict as $codepoint => $glypharray){
